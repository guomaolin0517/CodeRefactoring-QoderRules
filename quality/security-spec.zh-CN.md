...
| 防护类型 | 要求 |
|----------|------|
| **防 CSRF 攻击** | 非 GET 请求需校验 CSRF Token（如 `X-XSRF-TOKEN`） |
| **防 XSS 攻击** | 前端输入数据过滤 HTML 标签、特殊字符，后端存储 / 返回时进行转义 |
| **防请求重放** | 关键接口（支付、下单、数据提交）需添加时间戳 + 签名校验，**禁止**重复请求 |
| **接口限流** | 核心接口（登录、下单、查询）需限流（如每秒 5 次 / 用户），避免恶意请求压垮系统 |
...
# 版本历史
# ============================================
# v1.0 (2026-03-11) - 初始安全规范，包含 12 条规则
# ============================================
